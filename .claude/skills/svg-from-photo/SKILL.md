...
- **Use the MEDIAN single-letter height, not the union bbox height, for font-size.** Even after the letter-shape filter, occasional outliers (a tall capital, a descender) still distort the union. `font-size = round(median(letter_heights) / 0.72)` is robust.
