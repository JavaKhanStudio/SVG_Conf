...
**SVGO with workshop-safe config as the final step.** vtracer emits coordinates with up to 17 decimal places; the resulting path strings are 5-10x bigger than they need to be without any visible quality difference. Run SVGO before committing — but with `inlineStyles`, `convertStyleToAttrs`, and `minifyStyles` disabled. The first two move the `:root` variables into a root-level `style` attribute, which the workshop's `:root { }` parser doesn't see; every `var(--xxx)` then goes unresolved and resvg falls back to black. The third sometimes rewrites class selectors with `:is(...)` which the workshop's regex parser also doesn't handle. Config lives at `tools/svgo.config.mjs`. With `floatPrecision: 1` plus the safe overrides, average reduction across the gallery was ~85 % (puppy 1.4 MB → 182 KB, leprechaun 1.0 MB → 174 KB, stone 16 MB → 2 MB) with metric drift under 0.01 IoU.
