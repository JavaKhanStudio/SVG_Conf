...
- **Paint a clean background rect under each text overlay.** Anti-aliased text edges in the source get partially bucketed into neighbouring categories (edge pixels of white text on a dark bg aren't pure white, so some land in other buckets). Hiding the main text-colour bucket leaves those edge fragments as visible halos. For each derived cluster, emit a `<rect>` filling the cluster's *pre-filter* bbox (slight padding) with a dedicated `--<area>-bg-color` CSS variable, placed in document order between the trace paths and the text overlay. Gated by the same `--text-overlay` variable so toggling to traced-text mode also hides the rects and reveals the original look. Makes the substituted text crisp and readable instead of sitting on fuzz.
